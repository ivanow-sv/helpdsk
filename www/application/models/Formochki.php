...
class Formochki extends Zend_Form
{
	// фильтр символов для TEXTAREA - "все кроме указанных"
	private $regexp_textarea="#[^\s0-9a-zа-я\.,:\"\(\);\-№]#ui";
	// для валидаторов - все перечисленные
	private $regexp_text4Valid="#[\s0-9a-zа-я\.,:\"\(\);\-№]#ui";
	...
	public function init()
	{
        ...
        $translator = new Zend_Translate('array', APPLICATION_PATH.'/language/' . 'Zend_Validate.php');
	...
	public function getRegExpTextarea() {
		return $this->regexp_textarea;
		;
	}

	public function setRegExpTextarea($pattern) {
		$this->regexp_textarea=$pattern;
		;
	}

	public function getRegExpText4Valid() {
		return $this->regexp_text4Valid;
		;
	}

	public function setRegExpText4Valid($pattern) {
		$this->regexp_text4Valid=$pattern;
		;
	}
    ...
    "'%value%' contains characters which are not digits; but only digits are allowed" => "Значение '%value%' должно содержать только цифровые символы",
    "'%value%' is an empty string" => "'%value%' - пустая строка",
